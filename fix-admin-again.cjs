...
const regex = /app\.get\("\/api\/admin\/merchants", requireAdmin, async \(_req, res\) => \{[\s\S]*?res\.json\(enriched\);\r?\n\s*\}\);/;
...
const replacement = `app.get("/api/admin/merchants", requireAdmin, async (_req, res) => {
  try {
    ...
    const enriched = await Promise.all(usersRes.rows.map(async (u) => {
      ...
      // Calculate total volume by joining bank_accounts
      const volRes = await pool.query("SELECT COALESCE(SUM(o.amount), 0) as total FROM orders o JOIN bank_accounts b ON o.bank_account_id = b.id WHERE b.user_id = $1 AND o.status = 'PAID'", [u.id]);
